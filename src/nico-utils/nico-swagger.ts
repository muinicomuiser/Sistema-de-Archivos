...
export class DocumentadorSwagger {
  ...
  constructor(aplicacion: INestApplication, rutaDocumentacion: string = 'api') {
  ...
  static async documentador(
    ...
    const documentador: DocumentadorSwagger = new DocumentadorSwagger(
      aplicacion,
      rutaDocumentacion,
    );
   ...
   *
   * Por defecto considera todos los módulos de la aplicación.
   */
  ...
  packageJson(opcionesConfiguracion?: OpcionesConfig) {
    const configService: ConfigService =
      this.aplicacion.get<ConfigService>(ConfigService);
    ...
    if (this.opcionesConfig.description) {
      this.config.info.description = configService.get(
    ...
    if (this.opcionesConfig.authorName) {
      this.config.info.contact.name = configService.get(
        'npm_package_author_name',
      );
    }
    if (this.opcionesConfig.authorUrl) {
      this.config.info.contact.url = configService.get(
        'npm_package_author_url',
      );
    }
    if (this.opcionesConfig.authoEmail) {
      this.config.info.contact.email = configService.get(
  ...
  async logsRutas(url?: string) {
    ...
    NicoLogs.anuncioValor(
      'DOCUMENTACIÓN EN LA RUTA:',
      `${this.url}/${this.rutaDocumentacion}`,
      { colorAnuncio: 'verde', colorValor: 'blanco' },
    );
    this.subRutasRegistradas.forEach((subRutaRegistrada) => {
      NicoLogs.anuncioValor(
        'DOCUMENTACIÓN DE MÓDULO EN LA RUTA:',
        `${this.url}/${this.rutaDocumentacion}${subRutaRegistrada}`,
        { colorAnuncio: 'verde', colorValor: 'blanco' },
      );
    });
  }
  ...
  documentar(opciones?: { logRuta?: boolean }) {
    ...
    SwaggerModule.setup(
      `${this.rutaDocumentacion}${this._subRuta}`,
      this.aplicacion,
      SwaggerModule.createDocument(this.aplicacion, this.config, {
