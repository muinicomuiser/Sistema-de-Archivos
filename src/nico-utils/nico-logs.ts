...
export class NicoLogs {
  ...
  static puerto(puerto: number | string): void {
    console.log(
      `${this.colorFuente.amarillo}${this.estilo.brillo}[*NICO-LOGS*] - ${this.colorFuente.azul}SERVICIO ACTIVO EN EL PUERTO ${this.colorFuente.amarillo}${puerto}`,
    );
  ...
  static anuncioValor(
    anuncio: string,
    valor: number | string,
    opcionesColor?: OpcionesEstilo,
  ): void {
    let opciones: OpcionesEstilo = opcionesColor
      ? opcionesColor
      : { colorAnuncio: 'azul', colorValor: 'amarillo', estilo: 'brillo' };
    ...
    console.log(
      `${colorLog}${estiloLog}[*NICO-LOGS*]${colorAnuncio}   ${anuncio} ${colorValor}${valor}${this.estilo.reset}`,
    );
  }

  static rutaLocalHost(
    anuncio: string,
    ...
    const ruta: string =
      path != undefined
        ? `http://localhost:${puerto}/${path}`
        : `http://localhost:${puerto}`;

    console.log(
      `${colorLog}${estiloLog}[*NICO-LOGS*]${colorAnuncio}   ${anuncio} ${colorValor}${this.estilo.subrayado}${ruta}${this.estilo.reset}`,
    );
  }
}
